			...
			<tbody>
				<tbody>
					...
					foreach ($_SESSION["bproducts"] as $key => $product) { //loop though items and prepare html content
						...
						$menuitem = $product["menuitem"];
						$item_id = $product["item"];
						$stockitem_id=$product["stockitem_id"];
                        $getmeasure = mysqli_query($con, "SELECT * FROM inventoryitems WHERE status=1 AND inventoryitem_id ='$item_id'");
                        $row1 = mysqli_fetch_array($getmeasure);
                        $measurement_id = $row1['measurement_id'];
						$product_qty = $product["quantity"];
					...
					?>
						<tr>
							...
							<td><a href="#" class="remove-item text-danger" data-key="<?php echo $key ?>" data-code="<?php echo $stockitem_id; ?>"><i class="fa fa-trash"></i></a></td>
						</tr>
					<?php } ?>
...
if (isset($_POST["load_cart"]) && $_POST["load_cart"] == 3){
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td><a href="#" class="remove-item2 text-danger" data-code="<?php echo $key; ?>"><i class="fa fa-trash"></i></a></td>
			</tr>
		<?php } ?>
...
if (isset($_GET["remove_item_code"]) && isset($_SESSION["bproducts"])) {
	$item_id   = filter_var($_GET["remove_item_code"], FILTER_SANITIZE_STRING); //get the product code to remove
...
if (isset($_GET['remove_items'])&&isset($_SESSION['bproducts'])) {
	$stockitem = filter_var($_GET['remove_items'], FILTER_SANITIZE_STRING);
	$item_id = filter_var($_GET['item_id'], FILTER_SANITIZE_STRING);
	// unpdate stock item order 
	$change = mysqli_query($con, "UPDATE restockitems SET status='4' WHERE restockitem_id ='$stockitem'") or die(mysqli_error($con));
	if (isset($_SESSION["bproducts"][$item_id])){
		unset($_SESSION["bproducts"][$item_id]);
	}
	$total_items = count($_SESSION["bproducts"]);
