...
?>
        ...
        <div class="content-body">
			...
			<div class="container-fluid">
       		...
       		<div class="row">
                    <div class="col-lg-12">
                        <a href="scannerreportprint?id=<?php echo $id; ?>" class="btn btn-success mb-2">Print Report</a>
                    </div>
                      <?php      
                                    $checkscanner= mysqli_query($con, "SELECT * FROM scannerreports WHERE scannerreport_id='$id' AND status=1") or die(mysqli_error($con));
                                    $rowe= mysqli_fetch_array($checkscanner);
                               $scannerreport_id=$rowe['scannerreport_id'];
      ...
      $pin=$patient_id;
        ...
        $getgroups= mysqli_query($con,"SELECT * FROM agegroups WHERE status=1 AND agegroup_id='$agegroup'") or die(mysqli_error($con));
                                           $row1= mysqli_fetch_array($getgroups);
                                                ...
                                                $code1=$row1['code'];
               $checkscanner= mysqli_query($con, "SELECT * FROM scannerreports WHERE scannerreport_id='$id' AND status=1") or die(mysqli_error($con));
                                 $row3= mysqli_fetch_array($checkscanner);
               $month=$row3['month'];
               $year=$row3['year'];
               $reason=$row3['reason'];
