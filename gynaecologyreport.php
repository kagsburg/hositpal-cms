...
<head>
    ...
    <title>Gynaecology Report</title>
    <!-- Favicon icon -->
...
if((isset($_SESSION['lan']))&&($_SESSION['lan']=='fr')){ 
                                           include 'fr/gynaecologyreport.php';                     
                                       }else{
?>
        ...
        <div class="content-body">
			...
			<div class="container-fluid">
       		...
       		<div class="row">
                    <div class="col-lg-12">
                        <a href="gynaecologyreportprint?id=<?php echo $id; ?>" class="btn btn-success mb-2">Print Report</a>
                    </div>
                      <?php      
                               $checkgynaecology= mysqli_query($con, "SELECT * FROM gynaecologyreports WHERE gynaecologyreport_id='$id' AND status=1") or die(mysqli_error($con));
                              $rowe= mysqli_fetch_array($checkgynaecology);
                            $gynaecologyreport_id=$rowe['gynaecologyreport_id'];
      ...
      $pin=$patient_id;
        ...
        $getgroups= mysqli_query($con,"SELECT * FROM agegroups WHERE status=1 AND agegroup_id='$agegroup'") or die(mysqli_error($con));
                                           $row1= mysqli_fetch_array($getgroups);
                                                ...
                                                $code1=$row1['code'];
               $checkgynaecology= mysqli_query($con, "SELECT * FROM gynaecologyreports WHERE gynaecologyreport_id='$id' AND status=1") or die(mysqli_error($con));
               $row3= mysqli_fetch_array($checkgynaecology);
               $month=$row3['month'];
               $year=$row3['year'];
               $reason=$row3['reason'];
